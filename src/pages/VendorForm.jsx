...
export default function VendorForm({ org, editing, onSave, onCancel }) {
  ...
  async function save() {
    ...
    try {
      ...
      const res = await fetch(url, { method, headers: authHeaders(), body: JSON.stringify({ ...form, type:'Vendor' }) });
      const j = await res.json().catch(() => ({}));
      if (!res.ok || j.success === false) { alert(j.message || 'Could not save the vendor.'); return; }
      onSave();
