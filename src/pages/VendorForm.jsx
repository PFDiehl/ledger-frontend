...
export default function VendorForm({ org, editing, onSave, onCancel }) {
  ...
  async function save() {
    ...
    try {
      if (editing?.id) {
        await fetch(`${API}/orgs/${org.id}/contacts/${editing.id}`, { method:'PATCH', headers: authHeaders(), body: JSON.stringify({ ...form, type:'Vendor' }) });
      } else {
        await fetch(`${API}/orgs/${org.id}/contacts`, { method:'POST', headers: authHeaders(), body: JSON.stringify({ ...form, type:'Vendor' }) });
      }
      onSave();
  ...
  return (
    <div style={{padding:32,maxWidth:600,margin:'0 auto'}}>
      ...
      <h1 style={{margin:'0 0 32px',fontSize:26,fontWeight:700}}>{editing?.id ? 'Edit Vendor' : 'New Vendor'}</h1>
      ...
      <div style={{background:'var(--color-surface)',borderRadius:14,padding:28,border:'1px solid var(--color-border)',marginBottom:20}}>
        ...
        <div style={fieldStyle}><label style={labelStyle}>Company</label><input style={inputStyle} value={form.company||''} onChange={set('company')} placeholder="Acme Supply Co." /></div>
