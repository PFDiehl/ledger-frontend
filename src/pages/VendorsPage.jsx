...
export default function VendorsPage({ org }) {
  ...
  useEffect(() => { if (org) loadVendors(); }, [org]);
  ...
  const filtered = Vendors.filter(c => c.name.toLowerCase().includes(search.toLowerCase()) || (c.email||'').toLowerCase().includes(search.toLowerCase()));
        ...
        <div style={{background:'var(--color-surface)',borderRadius:14,border:'1px solid var(--color-border)',overflow:'hidden'}}>
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:14}}>
            <thead><tr style={{background:'var(--color-surface-secondary)'}}>
              {['Vendor','Email','Phone','Salesperson','Total Invoiced','Outstanding',''].map(h=>(
